...
class BackupSettingsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnBackupNow).setOnClickListener {
            ...
            if (!session.anyBackupEnabled()) {
                Toast.makeText(this, "Önce galeri yedeğini açın", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
    ...
    private fun refreshStatus() {
        ...
        backupStatus.text =
            "Durum: $on · $net$wifiNow · İşlenen medya: $count · Yer açılabilir: $freeable · Ek klasör: $folders$oem$partial\n$last\n" +
                "Galeri → TR Photos · klasörler → TR Backup / ${session.deviceName}"
        networkHint.text =
            "Yedekleme yalnız Wi‑Fi ile yapılır (fotoğraf, video ve seçilen klasörler)."
    }
